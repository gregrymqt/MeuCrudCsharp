...
public class PlanRepository : IPlanRepository
{
    ...
    public async Task AddAsync(Plan plan) => await _context.Plans.AddAsync(plan);

    public void Update(Plan plan) => _context.Plans.Update(plan);

    public async Task<int> SaveChangesAsync() => await _context.SaveChangesAsync();

    public void Remove(object payload) => _context.Remove(payload);
    ...
    {
        var query = _context.Plans.AsNoTracking().Where(p => p.IsActive == true);
    ...
    public async Task<List<Plan>> GetByExternalIdsAsync(IEnumerable<string> externalIds) =>
        await _context.Plans.Where(p => externalIds.Contains(p.ExternalPlanId)).ToListAsync();
}
