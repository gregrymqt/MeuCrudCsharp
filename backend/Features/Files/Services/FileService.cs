...
public class FileService : IFileService
{
    ...
    {
        // Regra absoluta: Tudo fica dentro de "uploads" [cite: 3]x
    ...
    public async Task<EntityFile> SubstituirArquivoAsync(int idArquivoAntigo, IFormFile novoArquivo)
    {
        try
        {
            ...
            if (arquivoBanco == null)
                throw new ResourceNotFoundException(
                    $"Arquivo ID {idArquivoAntigo} não encontrado para substituição."
                );
            ...
            if (novoArquivo != null && novoArquivo.Length > 0)
            {
                ...
                string novoCaminhoFisico = GerarCaminhoFisico(
        ...
        catch (Exception ex)
        {
            throw new AppServiceException(
                $"Erro ao substituir o arquivo ID {idArquivoAntigo}.",
                ex
            );
        }
    }
