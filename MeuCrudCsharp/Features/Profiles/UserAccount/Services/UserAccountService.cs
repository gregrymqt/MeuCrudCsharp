...
namespace MeuCrudCsharp.Features.Profiles.UserAccount.Services
{
    public class UserAccountService : IUserAccountService
    {
        ...
        public UserAccountService(ApiDbContext context, IMercadoPagoService mercadoPagoService, ICacheService cacheService)
        {
            _context = context;
        ...
        {
            var subscription = await _context.Subscriptions.FirstOrDefaultAsync(s => s.UserId == userId && s.Status == "pausado");
            if (subscription == null) return false;

            var result = await _mercadoPagoService.UpdateSubscriptionStatusAsync(subscription.ExternalId, "authorized");
