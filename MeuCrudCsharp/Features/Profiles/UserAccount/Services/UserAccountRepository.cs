...
public class UserAccountRepository : IUserAccountRepository
{
    ...
    public async Task<Subscription?> GetActiveSubscriptionByUserIdAsync(string userId) =>
        await _context.Subscriptions
            .Include(s => s.Plan) // Incluir o plano é importante para os detalhes
            .FirstOrDefaultAsync(s => s.UserId == userId && (s.Status == "active" || s.Status == "paused"));

    public async Task<IEnumerable<Payments>> GetPaymentHistoryByUserIdAsync(string userId, int pageNumber, int pageSize) =>
