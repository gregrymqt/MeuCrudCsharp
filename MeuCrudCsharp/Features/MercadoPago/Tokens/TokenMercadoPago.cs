...
namespace MeuCrudCsharp.Features.MercadoPago.Tokens
{
    public class TokenMercadoPago
    {
        ...
        public TokenMercadoPago(IConfiguration configuration)
        {
            ...
            {
                throw new ArgumentNullException(nameof(_access_Token), "O 'MERCADOPAGO_ACCESS_TOKEN' não foi encontrado na configuração.");
            }

            if (string.IsNullOrEmpty(_webhook_Secret))
            {
                throw new ArgumentNullException(nameof(_webhook_Secret), "O 'MERCADOPAGO_WEBHOOK_SECRET' não foi encontrado na configuração.");
            }
        }
    }
}
