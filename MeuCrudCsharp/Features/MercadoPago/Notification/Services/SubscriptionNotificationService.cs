...
namespace MeuCrudCsharp.Features.MercadoPago.Notification.Services
{
    public class SubscriptionNotificationService : ISubscriptionNotificationService
    {
        ...
        public SubscriptionNotificationService(ILogger<SubscriptionNotificationService> logger,
        ...
        {
            _logger.LogInformation("Iniciando regras de negócio para renovar a assinatura {SubscriptionId}.",
                subscription.Id);
            ...
            var viewModel = new RenewalEmailViewModel(
                ...
                transactionAmount: subscription.Plan.Price,
                accountUrl: "https://seusite.com/minha-conta",
                supportUrl: "https://seusite.com/suporte"
            );
    
            await SendEmailFromTemplateAsync(
                recipientEmail: subscription.User.Email,
                subject: "Sua assinatura foi renovada com sucesso!",
                viewPath: "Features/Shared/EmailTemplates/Renewal/index",
                model: viewModel
            );
            ...
            TModel model) 
        {
            ...
            try
            {
                ...
                var plainTextBody = $"Assunto: {subject}. Para visualizar esta mensagem, utilize um leitor de e-mail compatível com HTML.";

                await _emailSenderService.SendEmailAsync(recipientEmail, subject, htmlBody, plainTextBody);

                _logger.LogInformation("E-mail para {RecipientEmail} enviado com sucesso.", recipientEmail);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Falha no processo de montagem e envio de e-mail para {RecipientEmail}.", recipientEmail);
                throw;
            }
        }
    }
