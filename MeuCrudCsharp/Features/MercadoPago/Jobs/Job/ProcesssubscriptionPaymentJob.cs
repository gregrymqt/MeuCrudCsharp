...
namespace MeuCrudCsharp.Features.MercadoPago.Jobs.Job
{
    ...
    public class ProcessSubscriptionPaymentJob : IJob
    {
        ...
        public async Task ExecuteAsync(string resourceId)
        {
            if (string.IsNullOrEmpty(resourceId))
            {
                _logger.LogError("ResourceId (ID do Pagamento) está nulo ou vazio. O job não será reprocessado.");
                return;
            }
            ...
            try
            {
                ...
                var subscription = await _context.Subscriptions
                    .FromSqlRaw("SELECT * FROM \"Subscriptions\" WHERE \"PaymentId\" = {0} FOR UPDATE", resourceId)
                ...
                if (subscription == null)
                {
                    ...
                    _logger.LogWarning("Nenhuma assinatura encontrada para o pagamento com ID: {ResourceId}. Finalizando job.", resourceId);
                ...
                _logger.LogInformation("Job para pagamento {ResourceId} da assinatura {SubscriptionId} concluído com sucesso.", resourceId, subscription.Id);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erro ao processar o pagamento {ResourceId}. A transação será revertida.", resourceId);
