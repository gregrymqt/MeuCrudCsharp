...
namespace MeuCrudCsharp.Features.Courses.Services
{
    public class CourseService : ICourseService
    {
        ...
        public CourseService(ApiDbContext context, ILogger<CourseService> logger, ICacheService cacheService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<List<CourseDto>> GetAllCoursesWithVideosAsync()
        {
            ...
            {
                _logger.LogInformation("Buscando todos os cursos do banco de dados (cache miss).");
        ...
        public async Task<CourseDto> CreateCourseAsync(CreateCourseDto createDto)
        {
            try
            {
                ...
                return new CourseDto { Id = newCourse.Id, Name = newCourse.Name, Description = newCourse.Description };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erro inesperado ao criar o curso '{CourseName}'.", createDto.Name);
        ...
        public async Task<CourseDto> UpdateCourseAsync(Guid id, UpdateCourseDto updateDto)
        {
            ...
            if (course == null)
            {
                throw new ResourceNotFoundException($"Curso com ID {id} não encontrado para atualização.");
            }
            ...
            return new CourseDto { Id = course.Id,
                Name = course.Name,
        ...
        public async Task DeleteCourseAsync(Guid id)
        {
            var course = await _context.Courses
                .Include(c => c.Videos) // Inclui os vídeos para a verificação
                .FirstOrDefaultAsync(c => c.Id == id);

            if (course == null)
            {
                throw new ResourceNotFoundException($"Curso com ID {id} não encontrado para deleção.");
            }

            // REGRA DE NEGÓCIO: Não permitir deletar curso com vídeos
            if (course.Videos.Any())
            {
                _logger.LogWarning("Tentativa de deletar o curso {CourseId} que possui {VideoCount} vídeos.", id, course.Videos.Count);
                throw new AppServiceException("Não é possível deletar um curso que possui vídeos associados. Por favor, remova os vídeos primeiro.");
            }
